...
var qty = {};
		...
		function add_barang(e){
			...
			if ($("#row tr td input[value='"+produk_id+"']").length == 0 && qtyItem.length == 0){
				var subtotal = jml * produk_harga;
					$("#row").append('<tr>'+
	                                '<td><input type="hidden" class="id" name="id_barang['+i+']" value="'+produk_id+'"><input type="text" name="sku_barang['+i+']" value="'+produk_kode+'" class="form-barang input-sm" required readonly></td>'+
	                                '<td><input type="text" name="nama_barang['+i+']" value="'+produk_nama+'" class="form-barang input-sm" required readonly></td>'+
	                                '<td>'+
	                                '<input type="hidden" name="hargatmp['+i+']" value="'+produk_harga+'" class="tmp_price'+produk_id+'">'+
	                                '<input type="text" name="harga['+i+']" value="'+produk_harga+'" class="form-control price input-sm price'+produk_id+'" onkeyup="update_price();" required>'+
	                                '</td>'+
	                                '<td>'+a+'</td>'+
	                                '<td><input type="number" name="diskon['+i+']" value="0" class="form-control input-sm disc'+produk_id+'" onkeyup="update_diskon('+produk_id+');" required></td>'+
	                                '<td><input type="number" name="quantity['+i+']" value="'+jml+'" class="form-control qty input-sm" id="qty'+produk_id+'" onkeyup="update_qty();" required></td>'+
	                                '<td><input type="text" name="subtotal['+i+']" value="'+subtotal+'" class="form-barang subtotal input-sm" id="sub'+produk_id+'" required readonly></td>'+
			            ...
			            success: function (result) {
			            	...
			            	if (el.val().length == result.sku_barang.length) {
							    if ($("#row tr td input[value='"+result.id_barang+"']").length == 0 && qtyItem.length == 0){
									$("#row").append('<tr>'+
					                                '<td><input type="hidden" class="id" name="id_barang['+i+']" value="'+result.id_barang+'"><input type="text" name="sku_barang['+i+']" value="'+result.sku_barang+'" class="form-barang input-sm" required readonly></td>'+
					                                '<td><input type="text" name="nama_barang['+i+']" value="'+result.nama_barang+'" class="form-barang input-sm" required readonly></td>'+
					                                '<td>'+
					                                '<input type="hidden" name="hargatmp['+i+']" value="'+result.harga_jual+'" class="tmp_price'+result.id_barang+'">'+
					                                '<input type="text" name="harga['+i+']" value="'+result.harga_jual+'" class="form-control price input-sm price'+result.id_barang+'" onkeyup="update_price();" required></td>'+
					                                '<td>'+a+'</td>'+
					                                '<td><input type="number" name="diskon['+i+']" value="0" class="form-control input-sm disc'+result.id_barang+'" onkeyup="update_diskon('+result.id_barang+');" required></td>'+
					                                '<td><input type="number" name="quantity['+i+']" value="'+jml+'" class="form-control qty input-sm" id="qty'+result.id_barang+'" onkeyup="update_qty();" required></td>'+
					                                '<td><input type="text" name="subtotal['+i+']" value="'+jml * result.harga_jual+'" class="form-barang subtotal input-sm" id="sub'+result.id_barang+'" required readonly></td>'+
					                                '<td><button type="button" class="btn btn-danger btn-sm del" onclick="hapus_row(this);"><i class="fa fa-trash"></i></button> </td>'+
					                            '</tr>');
					            					i++;
					                            total();
		...
		function detail_harga_penjualan(id_barang) {
			$.ajax({
				type : 'POST',
				url : 'detail_harga_penjualan/'+id_barang,
				dataType : "JSON",
				success:function(data){
					html = '';
					var i = 0;
		...
		function detail_harga_pembelian(id_barang) {
			$.ajax({
				type : 'POST',
				url : 'detail_harga_pembelian/'+id_barang,
				dataType : "JSON",
				success:function(data){
					html = '';
					var i = 0;
