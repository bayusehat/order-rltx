...
class Admin extends CI_Controller {
	...
	public function edit_detail_penjualan()
	{
		if($this->session->userdata('logged_in') == TRUE){
			...
			$c->unset_fields('id_barang','deleted','created','updated');
			$c->unset_add();
	...
	public function update_detail_penjualan($post_array,$primary_key)
	{
		...
		// Generate new Total
		$new_total = $this->db->query('SELECT SUM(subtotal) as total FROM tb_detail_penjualan WHERE id_penjualan='.$this->uri->segment(3))->result();
		foreach ($new_total as $data) {
			$total = $data->total;
		}
		$this->db->update('tb_penjualan',array('total' => $total),array('id_penjualan' => $this->uri->segment(3)));
	...
	public function update_detail_pembelian($post_array,$primary_key)
	{
		...
		$new_total = $this->db->query('SELECT SUM(subtotal) as total FROM tb_detail_pembelian WHERE id_pembelian='.$primary_key)->result();

		foreach ($new_total as $data) {
			$total = $data->total;
		}
		$this->db->update('tb_pembelian',array('total' => $total),array('id_pembelian' => $id_penjualan));
	               ...
	               foreach ($result as $row){
	               	if($row->stok > 0 || 1 == 1){ //no need pengecekan stock
	               		if($row->deleted == 0){
										 ...
										 echo '
										 <li>
		               			<a class="list" style="display:block;cursor:pointer" data-produk-id="'.$row->id_barang.'" data-produkkode="'.$row->sku_barang.'" data-produknama="'.$row->nama_barang.'" data-produkharga="'.$row->harga_jual.'" onclick="add_barang(this);">
					                <div class="row">
						                <div class="col-sm-6">
						               ' . $row->nama_barang . '
						                </div>
						                <div class="col-sm-6">
					                		<button type="button" class="add_cart btn btn-success btn-sm" data-produk-id="'.$row->id_barang.'" data-produkkode="'.$row->sku_barang.'" data-produknama="'.$row->nama_barang.'" data-produkharga="'.$row->harga_jual.'" data-jual="'.base_url().'index.php/admin/to_penjualan" data-beli="'.base_url().'index.php/admin/to_pembelian" onclick="add_barang(this);"><i class="fa fa-plus"></i></button>
	...
	public function detail_harga_penjualan($id_barang)
	{
		if($this->session->userdata('logged_in') == TRUE){
			$data = $this->Transaction_model->detail_harga_penjualan($id_barang);
	...
	public function detail_harga_pembelian($id_barang)
	{
		if($this->session->userdata('logged_in') == TRUE){
			$data = $this->Transaction_model->detail_harga_pembelian($id_barang);
	...
	public function cetak_detail_penjualan($id_penjualan)
	{
		if($this->session->userdata('logged_in')){
			...
			try {
				...
				$date = date("d M Y H:i:s", strtotime($invoice->tanggal_invoice));
