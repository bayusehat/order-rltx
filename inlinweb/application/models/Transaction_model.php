...
class Transaction_model extends CI_Model {
    ...
    public function nomor_jual()
    {
        ...
        if($query->num_rows() <> 0){      
              ...
              $nomor_jual_max = str_pad($kode, 5, "0", STR_PAD_LEFT);    
    ...
    public function nomor_beli()
    {
        ...
        if($query->num_rows() <> 0){      
              ...
              $nomor_beli_max = str_pad($kode, 5, "0", STR_PAD_LEFT);    
    ...
    public function simpan_retur_penjualan()
    {
   	...
   	public function update_retur_penjualan($id_retur_penjualan)
   	{
    	...
    	$qty_retur = $this->input->post('qty_retur');
    	$subtotal = $this->input->post('subtotal_retur');
    	$id_detail_retur_penjualan = $this->input->post('id_retur_detail_penjualan');
    	$id_detail_penjualan = $this->input->post('id_detail_penjualan');
    	

    	foreach($id_barang as $i => $item) { 

            $new_qty_jual = $qty_jual[$i]-$qty_ubah[$i];
            $detail[] = array(
                'id_barang' => $id_barang[$i],
                'sku_barang' => $sku_barang[$i],
                'nama_barang' => $nama_barang[$i],
    ...
    {
        return $this->db->select('tb_detail_penjualan.*,tb_penjualan.tanggal_penjualan')
    ...
    {
        return $this->db->select('tb_detail_pembelian.*,tb_pembelian.tanggal_pembelian')
    ...
    {
        return $this->db->select('tb_penjualan.*,tb_sales.nama_sales,tb_master_payment.nama_payment,tb_master_payment.cash_payment,tb_user.username,tb_pelanggan.nama_pelanggan,SUM(tb_retur_penjualan.total_retur) as total_retur_penjualan',FALSE)
                        ->from('tb_penjualan')
                        ->join('tb_sales','tb_sales.id_sales=tb_penjualan.id_sales')
                        ->join('tb_pelanggan','tb_pelanggan.id_pelanggan=tb_penjualan.id_pelanggan')
                        ->join('tb_master_payment','tb_master_payment.id_payment=tb_penjualan.id_payment')
    ...
    {
        return $this->db->select('tb_pembelian.*,tb_master_payment.nama_payment,tb_master_payment.cash_payment,tb_user.username,tb_supplier.nama_supplier,SUM(tb_retur_pembelian.total_retur_pembelian) as total_retur_pembelian',FALSE)
                        ->from('tb_pembelian')
                        ->join('tb_supplier','tb_supplier.id_supplier=tb_pembelian.id_supplier')
                        ->join('tb_master_payment','tb_master_payment.id_payment=tb_pembelian.id_payment')
