...
<table width="100%">
	...
	<tr>
		<td>Pembelian : <?= $detail->nomor_pembelian;?></td>
		<td>Supplier : <?= $detail->nama_supplier;?></td>
	</tr>
	<tr>
		<td>Tanggal : <?= date('d F Y H:i',strtotime($detail->tanggal_pembelian)); ?></td>
		<td>Jatuh Tempo : <?php
...
<table border="1" style="border-collapse: collapse;width: 100%" id="tb-detail">
	...
	<?php
	$cetak = $this->db->query('SELECT * FROM tb_detail_pembelian WHERE id_pembelian='.$detail->id_pembelian)->result();
		...
		echo '
			...
			<tr>
				...
				<td>Rp '.number_format($detail->total_retur_pembelian).'</td>
			</tr>-->
			<tr>
